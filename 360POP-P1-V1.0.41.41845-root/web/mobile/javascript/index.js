...
var mobile_host_control = {
    ...
    input_init: function () {
        $(document).find('input[type=text]').focus(function (ev) {
            var me = $(this);
            setTimeout(function () {
...
mobile_host_control.menu = {
    ...
    slide: function () {
        ...
        var selectObj = "#routerStatus,#routerSet";
        var menuChange = function (ev) {
            ev.preventDefault();
            var typeValue;
            ...
            node.menu.removeClass("active").eq(typeValue).addClass("active");
            node.content.stop(true, true);
            mobile_host_control.M_Swiper.subSlideTo(me.type,myHostSwiper);
            me.callback[me.type].call(me);
        };

        node.nav.undelegate(selectObj,"click").delegate(selectObj,"click",function(){
            var dataHtmlName = ["index_page","app_list"];
            var indexV = $(this).index();
            window.location.hash=dataHtmlName[indexV];
        });
        $(selectObj).unbind("myHostMenu").bind('myHostMenu',menuChange);
    },
...
//        show_message("save","加载中...");
        ...
        $.ajax({
            type: "get",
            url: "/mobile/" + html_name + ".htm?vn=" + new Date().getTime(),
            dataType: "html",
...
//                    hide_lock_div();$("#message_layer").hide();
    ...
    setTouchFn:function (data,key) {
        var appControl = mobile_host_control.app,map = languageM_current_navMap;
    ...
    router_setting_init: function () {
        var  m = this;
        var me = mobile_host_control;
        var map = languageM_current_navMap;
        if (!!me.firstLogin||!!me.menu.refreshFlag) {
            m.createAppMenu(map);
        }
    },
    _init_getCurrentMenuList:function(){
        var me = this;
        $.extend(true,languageM_current_navMap,languageM_nav_map);
        me.router_setting_init();
    },
    require_APP:function(){
        require([current_html],function(app){
             app.init();
        });
    },
    ...
    init: function () {
        ...
        this._init_getCurrentMenuList();//初始化菜单列表
        this.slide();
