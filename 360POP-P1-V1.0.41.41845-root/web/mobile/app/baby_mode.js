...
var baby_mode={
    ...
    initSettingBabyMode:function(type){
        ...
        if(!timeSlot.check(inputs.eq(0).val(),inputs.eq(1).val(),inputs.eq(2).val(),inputs.eq(3).val())){
            return false;
        }
    ...
    renderBabyModeWaitingList:function (data) {
        ...
        if (data && data.data) {
            ...
            for (var i = 0; i < tempLength; i++) {
                ...
                var full_name = removeHTMLTag(name);
                name = removeHTMLTag(GetDeviceNameStr(name));
                babyModeWaitingList += '<dd class="device-item select_dd ' + me.getDeviceType(temp[i].os_type) + '">' +
                '<span class="device-name" title=\'' + full_name + '\'>' + name + '</span>' +
