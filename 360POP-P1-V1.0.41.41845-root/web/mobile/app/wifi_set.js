...
var wireless_base={
    ...
    check_input_data:function(){
        reg_map.wireless_base=[
            ...
            {id:"wireless_key_val",type:$("#wls_ap_mode_sel").val()=="0"?"noneed":"password eq8_63"}
        ];
    ...
    wireless_base_set:function(){
        ...
        var getSameSetData = function(){
            $.extend(true,me.wireless_same_submit_data,{
                ...
                sec_set:{
                    ap_id:3,port_id:"WIFI2",SSID_broadcast:obj.SSID_broadcast,wpa_key:getAesString($("#wireless_key_val").val())
                }
            });
        };
    ...
    wireless_sta_mode:function(){
        $.post("/app/igd_wisp/wireless_sta_mode.cgi",{action:"get"},function(data){
            data = dataDeal(data);
            if(data.enable == "1" && data.status == "1"){
                $("#channel_width_sel,#channel_num_sel").addClass("selectDisabled");
            }
        });
    },
