...
var touch_link = {
	...
	bind_ctrl_event:function(){
		 $("#touch_link_nav dl").undelegate("dd","click").delegate("dd","click",function(){
		...
		$("#start_touch_link_btn").unbind("click").bind("click",function(){
			show_message("save");
	...
	paint_user_list:function(data){
		...
		if(data.length != 0){
			for(var i in data){
				...
				var $dd = $("<dd/>");
				$dd.attr("class","select_dd " + me.getDeviceType(tmp.os_type));
