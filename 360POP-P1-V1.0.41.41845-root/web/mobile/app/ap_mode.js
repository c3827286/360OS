...
var ap={
    ...
    //副ap无线设置初始化成功执行函数
     init_ap_set_callback:function(result){
        ...
        Tools.select.set(result.auth,"ap_ap_mode_sel");
        $("#ap_frm").show();
        me._setAPList_style();
    },
    ...
    create_ap_list:function(data){
        ...
        if(!!data&&!!data.length){
            ...
            apList.removeClass("noTableListDataTip").html(listHtml).show();
        }else{
            apList.addClass("noTableListDataTip").html(appHtml.noApListTip).show();
        }
        me.addApListEvent();
    },
...
var reg_app_map={
    ...
    ap_safe_frm:[
        {id:"ap_wpa_key",type:"password eq8_30"}
    ]
}
///////////////////////////////// 副ap无线设置
//无线状态click设置
