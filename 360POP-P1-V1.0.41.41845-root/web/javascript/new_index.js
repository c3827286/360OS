...
function init_page() {
    ...
    get_wan_ip().then(function(){
        ...
        console.log("get wan ip error!");detectStateChange();
    });
}
...
function loginOut(noRes) {
    !noRes?$.post("/router/login_exit.cgi", {}, function () {exit();}):exit();
    function exit(){
...
/* ===================================== by houbingyang  断网急救 end ================================*/
var welcome_wan_data = {};
(function () {
    var welcome_wan_mode = function (value) {
        $("#wan_mode").val(value);
        hide_msgbox();
        $("#i_wired_frm ul li").removeClass("current").eq(parseInt(value - 1, 10)).addClass("current");
        reg_map["i_wired_frm"][0] = {};
        reg_map["i_wired_frm"][1] = {};
        reg_map["i_wired_frm"][2] = {};
        reg_map["i_wired_frm"][3] = {};
        reg_map["i_wired_frm"][4] = {};
        if (value == 1) { //pppoe
            $("#pppoe_layer").removeClass("section_hide").addClass("section_show");
            $("#static_layer").removeClass("section_show").addClass("section_hide");
            $("#dymaic_layer").removeClass("section_show").addClass("section_hide");
            reg_map["i_wired_frm"][0].id = "wan_setup_user";
            reg_map["i_wired_frm"][0].type = "pppoe_string";
            reg_map["i_wired_frm"][1].id = "wan_setup_pass";
            reg_map["i_wired_frm"][1].type = "password_blank";
        }
        if (value == 2) { //dhcp
            $("#pppoe_layer").removeClass("section_show").addClass("section_hide");
            $("#static_layer").removeClass("section_show").addClass("section_hide");
            $("#dymaic_layer").removeClass("section_hide").addClass("section_show");
        }
        if (value == 3) { //static
            $("#dymaic_layer").removeClass("section_show").addClass("section_hide");
            $("#pppoe_layer").removeClass("section_show").addClass("section_hide");
            $("#static_layer").removeClass("section_hide").addClass("section_show");
            reg_map["i_wired_frm"][0].id = "wan_setup_ip";
            reg_map["i_wired_frm"][0].type = "ip";
            reg_map["i_wired_frm"][1].id = "wan_setup_mask";
            reg_map["i_wired_frm"][1].type = "mask";
            reg_map["i_wired_frm"][2].id = "wan_setup_gw";
            reg_map["i_wired_frm"][2].type = "getway";
            reg_map["i_wired_frm"][3].id = "wan_setup_dns1";
            reg_map["i_wired_frm"][3].type = "dns";
            reg_map["i_wired_frm"][4].id = "wan_setup_dns2";
            reg_map["i_wired_frm"][4].type = "dns noneed";
        }
        set_lock_size();
    };

    var init_wan_con_type = function () {
		var d = new Date();
        $.post("/router/wan_config_show.cgi", {
            ...
            var connecttype = 0;
            if (!common) connecttype = 1;
            else
                connecttype = get_connect_type(common.connect_type);

            $("#conntype_tip_text").html(language[language_type]["SELECT"]["wan_mode"][connecttype - 1]["txt"]);
            $("#wan_mode").val(connecttype);

            welcome_wan_mode(connecttype);

            //STATIC
            $("#wan_setup_ip").val(handle_ip(_static.ip));
            $("#wan_setup_mask").val(handle_ip(_static.mask));
            $("#wan_setup_gw").val(handle_ip(_static.gw));
            $("#wan_setup_dns1").val(handle_ip(_static.dns[0]));
            $("#wan_setup_dns2").val(handle_ip(_static.dns[1]));

            //PPPoE
            if(pppoe.user != ""){
            	$("#wan_setup_user").val(utf8to16(base64decode(pppoe.user)));
			}
			else
				$("#wan_setup_user").val("");
            $("#wan_setup_pass").val(getDAesString(pppoe.pass));
        });
    };

    var set_wan_con_type = function () {
        ROUTE_INFO.waiting = true;
        var obj = {};
        $.extend(obj, welcome_wan_data.COMMON);
        obj.mac = welcome_wan_data.COMMON.mac_current;
        var val = $("#wan_mode").val();
        if (val == "2") {
            obj.connect_type = "DHCP";
            obj.mtu = 1500;
        }
        if (val == "1") {
            if (!check_input("i_wired_frm"))
                return;
            else {
                obj.connect_type = "PPPOE";
                obj.mtu = 1480;
                obj.pppoe_conf = "AUTO";
				obj.user = base64encode(utf16to8($("#wan_setup_user").val()));
                obj.pass = getAesString($("#wan_setup_pass").val());
				var d = new Date();
				obj.b64 = d.getTime();
            }
        }
        if (val == "3") {
            if (!check_input("i_wired_frm"))
                ...
                var tmp_ip = "",
                    tmp_mask = "",
                    tmp_gw = "";
                tmp_ip = $("#wan_setup_ip").val();
                tmp_mask = $("#wan_setup_mask").val();
                tmp_gw = $("#wan_setup_gw").val();
                var return_val = check_ip_mask(tmp_ip, tmp_mask);
                if (return_val != true) {
                    show_differ_tip(return_val, "wan_setup_ip");
                    return false;
                }

                var return_val1 = check_getway_mask(tmp_gw, tmp_mask);
                if (return_val1 != true) {
                    show_differ_tip(return_val, "wan_setup_gw");
                    return false;
                }
                var return_val2 = check_ip_ip2_mask(tmp_ip, tmp_gw, tmp_mask);
                if (return_val2 != true) {
                    show_differ_tip(return_val2, "wan_setup_ip");
                    return false;
                }

                obj.connect_type = "STATIC";
                obj.mtu = 1500;
                obj.dns1 = $("#wan_setup_dns1").val();
                obj.dns2 = $("#wan_setup_dns2").val();
                obj.gw = $("#wan_setup_gw").val();
                obj.ip = $("#wan_setup_ip").val();
                obj.mask = $("#wan_setup_mask").val();
            }
        }
        show_message("save");
        $.post("/router/wan_config_set.cgi", obj, function (data) {
            ...
            if (data == "SUCCESS") {
                ...
                close_i_wired_popwin("i_wired_pop");
            } else {
    ...
    var show_i_wired_pop = function () {
        var i_wired_li_addEventList = function () {
            $("#i_wired_frm ul li").unbind("click").bind("click", function () {
                var valData = $(this).index() + 1;
                welcome_wan_mode(valData);
            });
        }();
        close_i_wired_popwin('i_wired_dns');
        show_pop_layer('i_wired_pop');
        $("#i_wired_pop").css("top", "85px");
    };
    window.welcome_wan_mode = welcome_wan_mode;
    window.init_wan_con_type = init_wan_con_type;
    window.set_wan_con_type = set_wan_con_type;
    window.close_i_wired_popwin = close_i_wired_popwin;
    window.show_i_wired_pop = show_i_wired_pop;
})();
/* ===================================== by houbingyang  断网急救 end ================================*/
    ...
    var hitchDiagnosis = {
        ...
        getWanConfigSetData:function(data,type,value,pppoePassFlag){
            ...
            switch (type){
                ...
                case "macClone":obj.mac= value||common.mac_clone;break;
        ...
        connect_type_check:function(){//上网方式诊断
            ...
            var check_status_isOk=function(failFn,type,timer){
                ...
                me.setTimeOutId(function(){
                    when.settle([detect_wan(),wan_config_show()]).then(function(values){
                        /*       wisp - > dhcp*/
                        if((current_wiredStatus.status==1&&current_wiredStatus.wisp.enable&&values[1]['value'].connectType=="DHCP")||values[1]['value'].connectType=="STATIC"||values[0]['value'].connectType.indexOf(values[1]['value'].connectType)>-1)
                            me.linkStatusInfoShow(3,1);
                        else{
                            var detectWan = values[0]['value'].connectNo;
...
(function () {
        ...
        for(var m in data ){
            if(m.match(/(WAN).+/g)){
                if(data[m].status>>>0 ===0) {
                    return {status:0,ip:data[m].ip};
                }
            }
        }
        return {status:data["WAN1"].status,ip:data["WAN1"].ip};
    };
    var getWiredStatusObjByData=function(data,addButtonEvent,noIpFn) {
        //目前暂时只支持单wan口
        var wiredStatusObj={};
        wiredStatusObj.status= getWanStatus(data[0]).status;
    ...
    var renderIndexInfo = function (data) {
		...
		var render_timer;
        when.settle([getRouterInfo(), getDeviceCount(), getWanSpeed(), getWiredStatus()]).then(function (values) {
            for (var i = 0, j = values.length; i < j; i++) {
                if (values[i]['state'] == 'fulfilled') {
                    $.extend(info, values[i]['value']);
                }
            }
    ...
    var init_index_page = function () {
		if(ROUTE_INFO.is_touch_link)
			$("div[data-html-name=touch_link]").addClass("app");
		else
			$("div[data-html-name=touch_link]").removeClass("app");
        //加载向导
        renderIndexInfo();
        add_index_eventList();
    };
...
(function () {
    ...
    Device_list_info.prototype.add_device_list = function (dataItem,type,guest,touchList,authList) {
        var me = this;
        var limitedSpeed = "",waittingAuthList="";
        if (dataItem.limit_up_speed * 1 > 0 || dataItem.limit_down_speed * 1 > 0) {
            limitedSpeed = "limited-speed";
        }
        if (guest==2&&($.inArray(dataItem.mac,touchList)<0 || $.inArray(dataItem.mac,authList)<0)) {
            waittingAuthList = "waitAuthList";
        }
        if(dataItem.device_label == "unknown")
            ...
            dataItem.os_type = "12";
        }else if(dataItem.name.indexOf("POWER4S")>-1){
            dataItem.os_type = "13";
        }
        if(guest!=3){
            dataItem.rssi="无";
        }else{
            dataItem.rssi = parseInt(dataItem.rssi,10)>=75?"优":parseInt(dataItem.rssi,10)>=50?"良":parseInt(dataItem.rssi,10)>=25?"差":"极差";
            dataItem.os_type = "12";
        }
        var name = dataItem.alias || dataItem.device_label || dataItem.name || L.unnamed_device;
        ...
        if (type == "device") {
            ...
            dataItem.waittingAuthList = waittingAuthList;
    ...
    Device_list_info.prototype.judge_touch_list = function (dataItem, type ,nodeName,guest){
        var me = this;
        json_ajax({
            url:"/app/touch_link/webs/ql_get_list.cgi",
            data:{},
            successFn:function(data){
                var list = data,len= list.length,i= 0,touchList=[],authList=[],k= 0,touchLen = dataItem.length,strHtml = "";
                for(;i<len;i++){
                    var curMac = list[i].mac.toUpperCase().replace(/:/gi,"-");
                    touchList.push(curMac);
                    if(list[i].mode==2){
                        authList.push(curMac);
                    }
                }
                for(;k<touchLen;k++){
                    strHtml+= me.add_device_list(dataItem[k],type,guest,touchList,authList)
                }
                $("#nav_device_list_touch_link " + nodeName).html(strHtml);
            }
        });
    };
            ...
            successFn: function (data) {
                ...
                if (type == "device") {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        var guestNo = data[i]["is_guest"] >>0;
                        if(data[i]["type"]&&data[i]["type"]==4){
                            guestNo=3;
                        }
                        switch (guestNo) {
                            case 0:
                                strHtml0 += me.add_device_list(data[i], type,guestNo);
                                break;
                            case 1:
                                strHtml1 += me.add_device_list(data[i], type,guestNo);
                                break;
                            case 2:
                                touchLinkList.push(data[i]);
                                break;
                            case 3:
                                strHtml3+= me.add_device_list(data[i], type,guestNo);
                                break;
                        }
                    }
                    me.judge_touch_list(touchLinkList,type,nodeName,2);
    ...
    Device_list_info.prototype.init_device_list_action = function () {
        ...
        $(".nav_device_list_top .device_black_change .tab-item").die().live("click", function () {
            ...
            if(parentEmt.outTimeId)
				clearTimeout(parentEmt.outTimeId);
			parentEmt.init();
            parentEmt.editing = false;
        });
        //限速输入框输入判定
        ...
        $(".set-black-list-button").die().live("click", function () {
            ...
            json_ajax({
                ...
                finalFn: function (data) {
                    if (data.err_no == '-1') {
                        show_message('error', L.not_cur_device_to_black);
                    }
                    return true;
                },
                successFn: function (data) {
	...
	Device_list_info.prototype.init = function () {
		var me = this;
		//var calleeFn = arguments.callee;
		 if (me.outTimeId) {
		...
		if (current_html == me.runningPage) {
			me.canReader = true;
			me.init_device_list();
			me.outTimeId = setTimeout(function(){
				me.init();
				//calleeFn();
			}, me.outTime);
		} else {
...
//0: NONE 1: Windows 2: iPhone 3: Android 4: Windows Phone 5:  6: Linux  7: 其他
    ...
    var init_nav_device_list = function () {
        when.settle([getTouchLinkEnable(), getIgdApEnable()]).then(function (values) {
            var info = {
                touchLinkEnable: false,
                igdApEnable: false
            };
            for (var i = 0, j = values.length; i < j; i++) {
                if (values[i]['state'] == 'fulfilled') {
...
//摩擦上网(tmp)
(function(){
	var init_touch_link = function () {
		$("body").css("background","#fdfdfd");
		$("#conner").hide();
		$("#touch_link_layer").fadeIn(1500,function(){
			$(this).removeClass("section_hide").addClass("section_show");
			set_btn_pos();
		});
    }
    window.init_touch_link = init_touch_link;
})();

function fix_sougou_scroll_bug(){
	if(navigator.userAgent.toLowerCase().indexOf('se 2.x')>-1){
		if (window.top.document.documentElement.clientHeight < window.top.document.documentElement.offsetHeight-4){
			window.top.$(document).scrollTop(2);
		}
	}
}