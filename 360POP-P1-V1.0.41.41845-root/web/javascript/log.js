...
function get_log_msg(data) {
    var msg_str = igd.log_info[data.event];
    ...
    var action_arr = msg_str.split("@");						//取@后面的action动作
    var action = data.action - 1;
    var action_str;
    ...
    for (i = 0; i < data.msg.length; i++) {
        ...
        if (msg_str.indexOf("[" + j + "]") > -1) {
            ...
            /*else if(data.event==="LOG_CONNECT_STANDARD_PPPOE" && data.msg.length>2){
                var infoPPPoe = data.msg[i];
                msg_str =tmp_arr[0] + ["[账号:<span title=\"","[接口:",""][i]+infoPPPoe+["\">"+[infoPPPoe.length>6?infoPPPoe.substr(0,6)+"...":infoPPPoe,infoPPPoe][i]+"</span>]","]",""][i]+tmp_arr[1];
            }*/
            else {
