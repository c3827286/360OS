...
function get_msgbox(id,type,val) {
    var _input;
    if (typeof(id) == "object") {
        _input = id;
    }
...
function check_ip_fourth(str){
    ...
    if(str==""){
        var ss = L.four_section_not_null;
        return ss;
    }
    if(str==0){
...
function check_ip_come(str, check_lan_ip_b,check_ip_c,no_loop,basic) {
	...
	if(!!basic == false){
	   ...
	   /* if (str2[3] == "0") {
		...
		if(window.location.hash.indexOf("vpn_client")>-1)
			check_ip_c=true;
...
//            $(this).find(".float_info").addClass("tab_selected");
    ...
    var detectStateChange = function () {
        ...
        switch (menuObj["type"]) {
            case "menu":
                ...
                }else if(id == "service_zone"){
                    $("#service_zone").addClass('current').siblings().removeClass('current');
                    //  PAGE_INFO.tab_page_name = id;
    ...
    var init_menu = function () {
        var tempFun = function(obj){
            ...
            //需要绘制TAB选项卡的在此添加
            if(id == "nav_addon")
                newUrlToken += id + "/fast_tools";
            else if(id == "nav_setting")
                newUrlToken += id + "/fast_setting";
            else
                newUrlToken += id;
            window.location.hash = newUrlToken;
        ...
        $("#sidebar_wrap li").unbind("click").bind("click", function () {
            $("#service_zone").removeClass("current");
            tempFun(this);
        });
...
function print_message_panel(title, message) {
    ...
    var left = (parseInt(document.documentElement.clientWidth) - _this.outerWidth()) / 2 + "px";
    var top = (parseInt(document.documentElement.clientHeight) - _this.outerHeight()) / 2 + "px";//fix chrome bug
    _this.css("left", left);
    _this.css("top", top);
...
function show_lock_div(opacity) {
    var isMobile = window.location.pathname.indexOf("mobile")>-1?true:false;
    remove_quick_lock_div();
    $("#lock_div").show();
    !isMobile&&$("#lock_div").css({
...
function ck_pwd(id, pwd) {
    ...
    if (id == "login_pwd") {
		...
		if(id == "wireless_2_4_key_val" && ROUTE_INFO.is_5G){
			$("#wireless_2_4_key_val_pwd_ck").css("top","245px");
		}
    }
    pwStrength(id, pwd);
}
...
function select_chose_set(sel_id, val, func) {
    ...
    if(current_html=="virtual"){
        ...
        collection = document.getElementById(sel_id);
    }
    if (collection) {
...
function get_rand_key(error_count, key_index, is_get, getRandKeyErrorNo) {
    ...
    $.ajax({
        ...
        async: false,
        error: function (XMLHttpRequest, textStatus) {
            getRandKeyIsError(getRandKeyErrorNo?getRandKeyErrorNo:1);
        },
	...
	function getRandKeyIsError(errorNo){
        if(errorNo>2){
          return  retObj["rand_key"] = "";
        }else {
            retObj = calleeFn(error_count, key_index, is_get,errorNo+1);
        }
    }
...
!(function () {
    window._init_install_plugin = function(){
        ...
        var obj={};
        obj.action="get_app_list";
        $.post("/router/app.cgi",obj,function(data){
            var res = dataDeal(data),_temp=[];
            if(res["is_plugin"]!="0"&&res.data){
                var pluginList = res.data,_len = pluginList.length,k=0;
                for(;k<_len;k++){
                    if(!!pluginList[k]["plugin_path"]||pluginList[k]["plugin_path"]!==""){
