...
function DoUpdate() {
    ...
    UPDATE = window.setInterval(update_iframe_check, 1000);
...
function init_password() {
    ...
    window.setTimeout(function () {
        $("#igd_webs_old_password,#igd_webs_password1,#igd_webs_password2").val("");
        $("#igd_webs_old_password,#igd_webs_password1,#igd_webs_password2").removeClass().addClass("input-text");
    }, 200);
    ...
    $("#show_input_psw").unbind("click").bind("click", function () {
        var pswValShow = {
            _node: null,
            _parent: null,
            curVal: null,
            strHtml: null,
            action: function () {
                var tmp = this, _html = tmp.strHtml;
                tmp._node.remove();
                if (tmp._node.attr("type") == "text") {
                    if (_html.indexOf("type=") < 0) {
                        _html = _html.replace(">", " type=text />");
                    }
                    _html = _html.replace(/type=("|)text("|)/, "type=\"password\"");
                }
                else {
                    if (_html.indexOf("type=") < 0) {
                        _html = _html.replace(">", " type=password />");
                    }
                    _html = _html.replace(/type=("|)password("|)/, "type=\"text\"");
                }
                tmp._parent.append($(_html).val(tmp.curVal));
            },
            init: function (id) {
                if (!$("#" + id).val() || $("#" + id).val() == "") {
                    return;
                ...
                this.strHtml = this._node[0].outerHTML;
                this.curVal = this._node.val();
                this.action();
            }
        };
        pswValShow.init("igd_webs_password1");
    });
}
//cgi value
...
function set_reboot_type(type) {
    if (type == "1") {
        $("#reboot_now_div").removeClass("section_hide").addClass("section_show");
        $("#reboot_timed_div").removeClass("section_show").addClass("section_hide");
    }
    else if (type == "2") {
        $("#reboot_now_div").removeClass("section_show").addClass("section_hide");
        $("#reboot_timed_div").removeClass("section_hide").addClass("section_show");
    }
}
...
function DosoftReboot() {
    hide_dialog();
    $.post("/router/reboot.cgi", {noneed: "noneed"}, function (data) {
        var data = dataDeal(data);
        if (data == "SUCCESS") {
    ...
    var get_wan_info = function () {
        ...
        var _wan_info = function () {
            $.post("/router/wan_config_show.cgi", {uiname: uiname}, function (data) {
                ...
                $("#routerInfo_wanMac").html(wan_data["mac_current"]);
