srouter = {};
//    页面点击事件统计新增 by lwj
srouter.ddCountNo=(function(){
    var device_type = window.location.pathname.indexOf("mobile")>-1?true:false;
    return {
        Device_Type:device_type,
        DHCP:device_type?"3007":"3025",
        PPPOE:device_type?"3006":"3024",
        STATIC:device_type?"3008":"3026",
        wifi:device_type?"3009":"3023"
    };
})();
srouter.init = {};
...
srouter.init.welcome.showTestPage = function () {
    ...
    // 倒计时15s
    var countDown = 6;
    $('.page-get-method .count-down').text(6);
...
srouter.init.net.submit = function () {
    ...
    srouter.init.common.ajax("/router/wan_config_show.cgi", {uiname: "WAN1"}, function (data) {
        user_fov_info.appInfo(srouter.ddCountNo[params.connect_type]);
        if (data && data.COMMON) {
...
srouter.init.wifi.show = function () {
    ...
    user_fov_info.appInfo(srouter.ddCountNo.wifi);
};
...
srouter.init.wifi.startCountDown = function () {
    ...
    wifi.countDownTimer = setInterval(function () {
        ...
        if (wifi.countDown == 0) {
            ...
            $(".page-end .wifi-name").text(wifi.nameElement.val());
...
srouter.init.wifi.checkWifiConnect = function () {
    ...
    var skipUrl = srouter.ddCountNo.Device_Type?"/login_mobile.htm":"/login_pc.htm";
...
srouter.init.wifi.checkForm = function () {
    ...
    if (name.length > 32) {
        showError("wifi-ssid", L.wifi_name_lt_32);
        return false;
    }
	var name_obj = {};
	name_obj.value = name;
	name_obj.maxLength = 32;
    ...
    if (pass.length < 8 || pass.length > 30) {
        showError("wifi-password", L.pwd_8_30);
        return false;
    }
    if (!wifi.samePassElement.prop("checked")) {
        ...
        if (adminPass.length < 8 || adminPass.length > 30) {
            showError("admin-password", L.pwd_8_32);
            return false;
        }
    }
    return true;
