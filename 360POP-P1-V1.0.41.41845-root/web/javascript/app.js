...
function onekeyboot() {
    var deferred = when.defer();
    when.settle([bootqh_360(), bootdns(), bootarp()]).then(function (values) {
...
function paint_nav_protection_list(app_data) {
    ...
    getSafeStateData().then(function (data) {
        ...
        var header = "<div id=\"app_intro\"><div id=\"app_img\"><img src=\"" + info.icon + "\"/></div><div id=\"app_intro_list\"><div class=\"app_info\"><span class=\"app_name\">" + info.appname + "</span><span class=\"app_version\">"+ L.version +":" + info.version + "</span><span class=\"app_author\">"+ L.issuer +":" + info.author + "</span></div><div id=\"app_brief_area\"><p>"+ L.app_intro +":" + info.description + "</p></div></div></div>";
...
function paint_nav_addon_list(app_data) {
    ...
    for (var k = 0; k < app_data.length; k++) {
        ...
        for (var i = 0; i < len; i++) {
            ...
            if (item&&!item["plugin_path"]) {
                tab_html_str += "<li " + isEnd + "><div class='ext-item " + item.appsign + "' id='" + item.appsign + "' data-type='"+ type_name[k] +"'>" +
                "<div class='menu_text'>" + item.appname + "</div>" +
                "<div class='float_info'><div class='detail'>" + " </div></div></div></li >";
            }else{
                tab_html_str += "<li " + isEnd + "><div style='background-image: url(\"/"+item["plugin_path"]+"/"+item.appsign+".png\")' class='ext-item " + item.appsign + "' id='" + item.appsign + "' data-type='"+ type_name[k] +"'>" +
                "<div class='menu_text'>" + item.appname + "</div>" +
                "<div class='float_info' style='background-image: url(\"/"+item["plugin_path"]+"/"+item.appsign+"_w.png\")'><div class='detail'>" + " </div></div></div></li >";
            }
        }
        if (len > 0) {
            tab_html_str += "</ul>"
...
function show_app_detail(appsign) {
    ...
    for(var i in tmp_data) {
        if(appsign == tmp_data[i].appsign) {
            ...
            if ($("#app_img").length) {
                var $detail_img = $("<img/>"),_logoPath ="/images/" +  tmp_data[i].appsign + "_b.png" ;
                if( tmp_data[i]["plugin_path"]){
                    _logoPath="/"+ tmp_data[i]["plugin_path"]+"/"+ tmp_data[i].appsign + "_b.png"
                }
            ...
            //添加应用页显示
            if(tmp_data[i]["plugin_path"]){//为上传app
...
function paint_app_set_title(data) {
    ...
    if(data["plugin_path"]){
        url="/"+data["plugin_path"]+"/webs/index.html";
    }
    var exist_flag = true;
    $.ajax({
        url: url,
    ...
    if (exist_flag&&!data["plugin_path"]) { //有配置页面且非添加应用
    ...
    $.post("/router/app.cgi",obj,function(data){
        ...
        if(data == "SUCCESS"){
            ...
            if(act == "start"){
                ...
                else if(data.err_info != undefined){
                    show_message("error",app_file_err_msg[data.err_info]);
                }
            }
            else
                show_message("error",igd.make_err_msg(data));
...
(function () {
    ...
    var getSafeStateData = function () {
        var deferred = when.defer();
        when.settle([getQh360(), getDns(), getArpOversee(), getWifipassstrength(), getSecurityPwdStrength()]).then(function (values) {
            for (var i = 0, j = values.length; i < j; i++) {
                if (values[i]['state'] == 'fulfilled') {
                    $.extend(app_list_safety, values[i]['value']);
                }
            }
            var DDOSMap = {
                "wanping_status": "",
