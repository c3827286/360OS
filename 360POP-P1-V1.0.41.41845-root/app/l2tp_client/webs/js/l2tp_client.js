...
function init_client_tab(data){
    client_set_data=[];
    var new_data=l2tp_client__dataChangeTo(data);
    cloneAll(data,client_set_data);
    //remove ,"工作模式","NAT"
    var tab = new libTable(appHtml.clientTabHead,new_data,1,10);
    print_table("l2tp_client_tab",tab,[{type:'edit',name:'client_modify',str:appCommonJS.btnTitle.edit},{type:'del',name:'client_del',str:appCommonJS.btnTitle.del}],false);
    nos.app.resizePage();
}
function l2tp_client__dataChangeTo(data){
...
function init_client_status_tab(data){
    ...
    var tab = new libTable(appHtml.clientStatusTabHead,new_data,1,10);
    print_table("l2tp_status_tab",tab,null,false);
    nos.app.resizePage();
}
