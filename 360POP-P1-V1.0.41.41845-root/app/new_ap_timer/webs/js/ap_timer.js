...
(function () {
    var ap_timer = {
        ...
        initTimeSlotList: function (list) {
            var tab = new libTable(appHtml.timeSlotTabHead, list, 1, 10);
            print_table("ap_timer_tab", tab, [
                {type: 'edit', name: 'time_slot_modify', str: appCommonJS.btnTitle.edit},
                {type: 'del', name: 'time_slot_del', str: appCommonJS.btnTitle.del},
                {type: 'status', name: 'time_slot_enable', str: function (index) {
...
function time_slot_del(index) {
    ...
    $.post("./ap_timer.cgi", delInfo, function (data) {
        save_wifi_ctrl_callback(dataDeal(data));
    });
}
