    ...
    var powerTimeSlot = {
        list: function (currentTop) {
            $.post("/app/radio_power/radio_power.cgi", {action: "get"}, function (data) {
                ...
                var tab = new libTable(appHtml.timeSlotTabHead, listsArr, 1, 10);
                print_table("power_progress_tab", tab, [
                    {type: 'edit', name: 'power_modify', str: appCommonJS.btnTitle.edit},
                    {type: 'del', name: 'power_del', str: appCommonJS.btnTitle.del},
                    {type: 'status', name: 'power_enable', str: function (index) {
...
function power_del(index) {
    ...
    $.post('/app/radio_power/radio_power.cgi', {action: "del", idx: list.idx}, function (data) {
        save_power_progress_callback(dataDeal(data),currentTop);
    });
}
