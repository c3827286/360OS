...
var reg_app_map = {
	...
	qos_advance_form:[
		...
		{id: "wan_setup_up_reserved1", type: "decimal"},
        {id: "wan_setup_down_reserved1", type: "decimal"}
	],
    qos_priority_form: [
        {id: "prio", type: "prio"},
...
function check_band_width(){
	...
	for(var i = 1; i <= g_wan_num; i++){
		...
		if(parentEmt.check_decimal(up_bandwidth).toString() != 'true' || parentEmt.check_decimal(down_bandwidth).toString() != 'true'){
			flag = false;
			break;
		}
	}
...
function init_qos_priority() {
    ...
    /*//用户组
    nos.app.net('/app/group_manager/webs/igd_group_show.cgi', 'group_type=0', init_user_group, null, false);
    //dns组
    nos.app.net('/app/group_manager/webs/igd_group_show.cgi', 'group_type=3', init_dns_group_sele, null, false);
    //时间组
    nos.app.net('/app/group_manager/webs/igd_group_show.cgi', 'group_type=4', init_time_group_ctrl, time_group_error, false);*/
...
/*    var fun_arr = [
        {type: "edit", name: "qos_priority_update",str:"修改"},
        {type: "del", name: "qos_priority_del",str:"删除"}
    ];*/
    var tab = new libTable(appHtml.qosPriorityHead, data_new, 1, 10);
    print_table("qos_priority_list", tab, null, false);
    nos.app.resizePage();
}
/**
 * 得到选中的用户组数据
