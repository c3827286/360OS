	...
	if(ret.err_no == "0" && ret["data"].length != 0){
		...
		for(var i = 0; i < len; i++){
			if(list[i].mode == 0 || list[i].mode == 4){
				var tmp = {};
				tmp.hostname = list[i].hostname || appHtml.unknownDevice;
				tmp.mac = list[i].mac;
		...
		if(new_data.length != 0){
			var tab = new libTable(appHtml.user_list,new_data,1,10);
			window.setTimeout(function(){
				print_table("auth_list_tab",tab,null,false);
			},200);
		}
		else{
			no_user_tab_dom();
	...
	if(ret.err_no == "0"){
		...
		for(var i = 0; i < len; i++){
			if(list[i].mode == 2){
				var tmp = {};
				tmp.id = index;
				tmp.name = list[i].hostname || appHtml.unknownDevice;
				tmp.mac = list[i].mac;
		...
		var tab = new libTable(appHtml.wireless_list,new_data,1,30);
		print_table("wireless_list_table",tab,null,false);
		nos.app.resizePage();
	}
	else{
