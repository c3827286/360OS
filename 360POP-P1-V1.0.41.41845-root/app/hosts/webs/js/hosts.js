...
(function(){
	...
	var host = {
		...
		fin_flag:null,//用来标识初始化是不是触发了滚动事件，触发则为true 
		err_index:[],
		bind_ctrl_event:function(){
			...
			$("#host").attr("wrap", "off").css({
				...
				if(e.type == "scroll" || e.type == "keydown"){
		...
		host_line_count:function(e){
			...
			me.sleep_timer = window.setInterval(function(){
				if($("#host_lineshow").scrollTop() != $("#host").scrollTop()){
					if(window.top.$("#message_layer").css("display") == "none" && $("#host_lineshow").scrollTop() != $("#host").scrollTop() &&  $("#host").scrollTop()-$("#host_lineshow").scrollTop() > me.ONE_PAGE_LEN*me.offset)
						show_message("wait",appCommonJS.controlMessage.s_wait);
					me.host_line_count_loop(e);
				}
				else{
		...
		host_line_count_loop:function(e){//this function use to slow down do-while loop
			if(e.type == "keydown"){
				...
				if(e.ctrlKey && (e.keyCode != "17" || e.keyCode != "65" || e.keyCode != "86" || e.keyCode != "67"))
					return;
			...
			if($("#host").scrollLeft() != 0 && e.type == "scroll"){
				return;
			}
			me.line_str += (me.total_line_len++) + "    \n";
			$("#host_lineshow").val(me.line_str);
